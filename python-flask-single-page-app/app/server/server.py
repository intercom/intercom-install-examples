...
def intercomSettings():
    ...
    result = hmac.new(os.environ.get("INTERCOM_SECRET_KEY"), logged_in_user["id"], hashlib.sha256).hexdigest()
    # 3. Return Intercom settings from endpoint
    return jsonify({"app_id" : os.environ.get("INTERCOM_APP_ID"), "user_id" : logged_in_user["id"], "user_hash" : result })
